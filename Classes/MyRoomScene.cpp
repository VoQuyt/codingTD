		...
		*room = document["Room"].GetString();
		...
		if (check != -1)
		{
			...
			if (Tool::currentPlayer->id == Tool::ConvertStringToInt(*Vec[0]) || Tool::currentPlayer->id == Tool::ConvertStringToInt(*Vec[1]))
			{
				...
				btn_Roomname->setTitleText(Tool::currentPlayer->room_name);
...
void MyRoomScene::onReceiveEvent_UpdateRoom(SIOClient* client, const std::string& data)
{
	...
	document.Parse<0>(data.c_str());
	//check name has member in json
	if (document.HasMember("Room") && document["Room"].GetString() == Tool::currentPlayer->room_name) {
		*room = document["Room"].GetString();
		check = room->find("_");
		if (check != -1)
		{
			...
			if (Tool::currentPlayer->id == Tool::ConvertStringToInt(*Vec[0]) || Tool::currentPlayer->id == Tool::ConvertStringToInt(*Vec[1]))
			{
				Tool::currentPlayer->room_name = to_string(Tool::currentPlayer->id);
				//CCLOG("room %s", Tool::currentPlayer->room_name.c_str());
...
void MyRoomScene::onReceiveEvent_Chase_Player(SIOClient* client, const std::string& data)
{
	...
	document.Parse<0>(data.c_str());
	//check name has member in json
	if (document.HasMember("Room") && document["Room"].GetString() == Tool::currentPlayer->room_name && to_string(Tool::currentPlayer->id) == document["CurrentId"].GetString())
	{
		Tool::Socket_Client->_client->emit("Update_Room", "{\"CurrentId\":\"" + to_string(Tool::currentPlayer->id) + "\", \"OppenontId\":\"" + to_string(Tool::opponentPlayer->id) + "\" ,\"Room\":\"" + Tool::currentPlayer->room_name + "\"}");
		Tool::opponentPlayer->id = 0;
		Tool::currentPlayer->room_name = "0";
...
void MyRoomScene::btn_Click(Ref *pSender, cocos2d::ui::Button::Widget::TouchEventType type) {
	...
	if (type == Widget::TouchEventType::ENDED) {
		...
		if (name == "btn_Play") {
			Tool::Socket_Client->_client->emit("_Ready_", "{\"Room\":\""+ (Tool::currentPlayer->room_name) + "\"}");
		}
		if (name == "btn_Rank")
		{
		...
		if (name == "btn_Chase")
		{
			Tool::Socket_Client->_client->emit("_Chase_Player_", "{\"OppenontId\":\"" + to_string(Tool::opponentPlayer->id) + "\", \"Room\":\"" + Tool::currentPlayer->room_name + "\"}");
			Tool::opponentPlayer->id = 0;
			//Director::getInstance()->replaceScene(ListRoomScene::createScene());
		}
		if (name == "btn_Exit")
		{
			string id = to_string(Tool::currentPlayer->id);
			if(Tool::currentPlayer->room_name.find("_") != -1)//nếu trong room name không có kí tự "_"
			{
				Tool::Socket_Client->_client->emit("Update_Room", "{\"CurrentId\":\"" + to_string(Tool::currentPlayer->id) + "\", \"OppenontId\":\"" + to_string(Tool::opponentPlayer->id) + "\" ,\"Room\":\"" + Tool::currentPlayer->room_name + "\"}");
				Tool::opponentPlayer->id = 0;
			}
			...
			{
				Tool::Socket_Client->_client->emit("Destroy_Room", "{\"id\":\"" + id + "\", \"Room\":\"" + Tool::currentPlayer->room_name + "\"}");
			}
			Tool::currentPlayer->room_name = id;
			Director::getInstance()->replaceScene(ListRoomScene::createScene());
		}
	}
}
...
void MyRoomScene::SetupGUI()
{
	...
	//lbl_RoomName = Tool::CreateLabel(to_string(Tool::currentPlayer->id), Tool::defaultTextSize*1.5);
	//lbl_RoomName = Label::createWithTTF(to_string(Tool::currentPlayer->id), "fonts/arial.ttf", 32);
	...
	btn_Roomname->setTitleText(to_string(Tool::currentPlayer->id));
