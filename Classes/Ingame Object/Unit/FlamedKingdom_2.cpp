#include "FlamedKingdom_2.h"

FlamedKingdom_2::FlamedKingdom_2(int line, bool isOwned, int unitId, int playerId)
{
...
FlamedKingdom_2::~FlamedKingdom_2()
{
}

void FlamedKingdom_2::Die()
{
...
void FlamedKingdom_2::Attack(vector<BaseUnitClass*>& targets)
{
	this->action = "Attack";
...
vector<BaseUnitClass*> FlamedKingdom_2::FindTargets()
{
	...
	for (auto target : shotableTargets) {
		...
		if (!isAffected) {
			target->ApplyStatus(StatusReceive("Burning Aura", "Regeneration", -14, Tool::currentIngameTime + 5, 69));
		}
	}
