#include "FrozenKingdom_2.h"

FrozenKingdom_2::FrozenKingdom_2(int line, bool isOwned, int unitId, int playerId)
{
...
FrozenKingdom_2::~FrozenKingdom_2()
{
}

void FrozenKingdom_2::Die()
{
...
void FrozenKingdom_2::Attack(vector<BaseUnitClass*>& targets)
{
	this->action = "Attack";
...
vector<BaseUnitClass*> FrozenKingdom_2::FindTargets()
{
	...
	for (auto target : shotableTargets) {
		...
		if (!isAffected) {
			target->ApplyStatus(StatusReceive("Frozen Aura", "AttackSpeed", 0.85, Tool::currentIngameTime + 5, 2));
		}
	}
